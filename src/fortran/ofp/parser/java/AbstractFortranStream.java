...
public abstract class AbstractFortranStream extends ANTLRStringStream implements IFortranStream {
	...
	public AbstractFortranStream() {
		super();
	}

	public AbstractFortranStream(String input) {
		super(input);
	}
	...
	protected void convertInputBuffer() {
		...
		char[] newData = new char[this.n+2];
	
		int from = 0, to = 0;
		while( from < this.n )
		{
			if (this.data[from] != '\r') {
				newData[to++] = this.data[from];
			}
			else {
				newData[to++] = '\n';  // replace '\r'
				if (from+1 < this.n && this.data[from+1] == '\n') {
	...
	protected boolean matchFreeFormCommentLine(int i, char buf[]) {
		...
		int i1 = i;
		while(i1 < this.n && buf[i1] == ' ') i1 += 1;
	
		if (i1 >= this.n) return false;
	 ...
	 */
	protected int consumeFixedFormComments(int i, char buf[], StringBuffer comments) {
		if (i < this.n && buf[i] == '!') {
			// found comment character, copy characters up to '\n'
			do {
				comments.append(buf[i++]);
	...
	protected int skipFreeFormContinuationAtBegin(int i, char buf[]) {
		...
		while (ii < this.n && buf[ii] == ' ' && buf[ii] != '&') ii += 1;
		if (buf[ii] == '&') i = ii + 1;
	...
	protected int[] consumeHollerith(int i, char buf[], int count, char newBuf[]) {
		...
		if (i >= this.n) return new int[] {i, count};
	...
	protected int[] completeContinuedString(char quoteChar, int i, char buf[], int count, char newBuf[]) {
		...
		// skip initial '&'
		if (++i >= this.n) return new int[] {i0, count};
		...
		// TODO - what about TABS?
		while (i < this.n && buf[i] == ' ') i += 1;
	
		if (i >= this.n) return new int[] {i-1, count};
		...
		do {
			...
			if (i < this.n - 1 && buf[i] == quoteChar && buf[i+1] == quoteChar) {
				newBuf[count++] = buf[i++];
				newBuf[count++] = buf[i++];
			}
		}
		while (i < this.n && buf[i] != quoteChar && buf[i] != '&' && buf[i] != '\n');
	...
	protected int[] consumeFreeFormString(int i, char buf[], int count, char newBuf[]) {
		...
		do {
			newBuf[count++] = buf[i++];
			// look for two quote chars in a row, if found copy both
			if (i < this.n - 1 && buf[i] == quote_char && buf[i+1] == quote_char) {
				newBuf[count++] = buf[i++];
				newBuf[count++] = buf[i++];
			}
		...
		while (i < this.n && buf[i] != quote_char && buf[i] != '&' && buf[i] != '\n');
	...
	protected int consumeFixedFormString(int i, char buf[], int count, char newBuf[]) {
		...
		while (i < this.n && buf[i] != quote_char && buf[i] != '\n');
